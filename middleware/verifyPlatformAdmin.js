...
const authenticatePlatfromAdmin = (req, res, next) => {
  ...
  jwt.verify(token, 'your_secret_key_here', (err, decodedToken) => {
    ...
    if (decodedToken.type !== 'PlatformAdmin') {
      return res.status(403).json({ error: 'Access forbidden. Platform Admin role required.' });
    }
...
export { authenticatePlatfromAdmin };